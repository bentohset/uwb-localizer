...
#define POLL_RX_TO_RESP_TX_DLY_UUS 81000
...
void generatePayload() {
  Serial.print("Payload generated: ");
  for (int i = 0; i < 100; i++) {
    tx_payload[i] = rand() % 256;
    Serial.print(tx_payload[i], HEX);
  }
  Serial.println();
}
...
// combines resp_msg and resp_payload into resp_frame
void generateFrame() {
  Serial.println("Generate frame, combine resp_msg and payload for 127bytes");
  for (int i = 0; i < sizeof(tx_resp_msg); i++) {
    tx_frame[i] = tx_resp_msg[i];
  }
  int payload_index = sizeof(tx_resp_msg);
  for (int i = 0; i < sizeof(tx_payload); i++) {
    tx_frame[payload_index + i] = tx_payload[i];
  }
}
  ...
  if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) {
    ...
    if (frame_len <= sizeof(rx_buffer)) {
      ...
      if ((memcmp(rx_buffer, rx_poll_msg1, ALL_MSG_COMMON_LEN) == 0) || (memcmp(rx_buffer, rx_poll_msg2, ALL_MSG_COMMON_LEN) == 0)) {
        ...
        // printArr(tx_resp_msg, sizeof(tx_resp_msg));
        // Serial.print("Frame is: ");
        // printArr(tx_frame, sizeof(tx_frame));

        dwt_writetxdata(sizeof(tx_frame), tx_frame, 0); /* Zero offset in TX buffer. */
        dwt_writetxfctrl(sizeof(tx_frame), 0, 1); /* Zero offset in TX buffer, ranging. */

        // dwt_writetxdata(sizeof(tx_resp_msg), tx_resp_msg, 0); /* Zero offset in TX buffer. */
        // dwt_writetxfctrl(sizeof(tx_resp_msg), 0, 1); /* Zero offset in TX buffer, ranging. */

        Serial.println("Attempting to send frame to anchor");
        ...
        if (ret == DWT_SUCCESS) {
          ...
          frame_seq_nb++;
          Serial.println("Frame successfully sent");
        } else {
          Serial.println("Frame exchange abandoned, code flow interrupted.");
        }
      }
    }
  } else {
