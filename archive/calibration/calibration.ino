...
#define A_DISTANCE 0.8

// Wifi details
const char *ssid = "TP-Link_3logytech";
const char *password = "3logytech1928";
const char *host = "192.168.0.145";   //ip address of host aka laptop for visualisation
WiFiClient client;
WiFiUDP udp;
...
static uint8_t tx_poll_msg[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'S', 'I', 'J', 'A', 0xE0, 0, 0};
...
void loop() {
  ...
  if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) {
    ...
    if (frame_len <= sizeof(rx_buffer)) {
      ...
      if (UDP_ENABLED) {
        add_link(uwb_data, convertToUint8(tx_poll_msg));
      }
